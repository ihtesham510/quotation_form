...
export function Step5QuotePreview({ quoteData, onSaveQuote, onGeneratePDF, onEmail, productDatabase }: Step5Props) {
	const selfContainedData: SelfContainedQuoteData = {
		...
		products: quoteData.products.map(product => {
			...
			const total = calculateProductTotal(product, quoteData.gstEnabled, quoteData.gstRate, productDatabase, quoteData)
	...
	return (
		<div className='space-y-6'>
			...
			<Card>
				<CardHeader>
					<CardTitle className='text-base'>Product Breakdown ( Incl. GST )</CardTitle>
				</CardHeader>
				<CardContent className='space-y-4'>
					{quoteData.products.length === 0 ? (
						<p className='text-muted-foreground'>No products added to this quote.</p>
					) : (
						quoteData.products.map((product, _) => {
							...
							return (
								<div key={product.id} className='border-b pb-2 last:border-b-0 last:pb-0'>
									<div className='flex justify-between items-start'>
										<div className='grid'>
											<h1 className='font-medium'>{product.label}</h1>
											<div className='font-medium'>
												{productInfo?.name || 'Unknown Product'} (Qty: {product.quantity})
											</div>
										</div>
									...
									<div className='text-sm text-muted-foreground'>
										...
										{product.controlType !== 'Cord' && ` • ${product.controlType}`}
									</div>
									...
									{product.specialFeatures && (
										<div className='text-xs text-muted-foreground italic'>Features: {product.specialFeatures}</div>
									)}
								</div>
							)
						})
			...
			<div className='flex justify-end space-x-4'>
				{onSaveQuote && (
					<Button variant='outline' onClick={() => onSaveQuote(selfContainedData)}>
						<SaveIcon className='size-4' />
						<p className='hidden md:inline-flex'>Save Quote</p>
					</Button>
				)}
				{onGeneratePDF && (
					<Button variant='outline' onClick={() => onGeneratePDF(selfContainedData)}>
						<DownloadIcon className='size-4' />
						<p className='hidden md:inline-flex'>Download PDF</p>
					</Button>
				)}

				{onEmail && <Button onClick={() => onEmail(selfContainedData)}>Email</Button>}
			</div>
		</div>
	)
}
